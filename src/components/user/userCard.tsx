...
export default class UserCard extends React.Component<IUserCardProps, any> {
  ...
  render() {
    ...
    let accoutIconList = map(accounts, (account: any, index) => {
      return (
        <AccountIcon key={index} info={account.info} type={account.type} />
      );
    });
    return (
      <div className="user-profile">
        ...
        <div className="account-icons">{accoutIconList}</div>
      </div>
    );
  }
}
...
class AccountIcon extends React.PureComponent<IAccountIconProps, any> {
  ...
  render() {
    ...
    return (
      <a className={`account-icon ${this.props.type}`} onClick={this.click}>

...
export default class UserController {
  ...
  public static async createUser(ctx: koa.Context) {
    ...
    if (!account) {
      ...
      url = `${url}/validate/change_email?uid=${data.id}&authcode=${getAuthcode(
        data.id
      )}`;
  ...
  public static async sendEmail(ctx: koa.Context) {
    ...
    if (type == "email") {
      title = "验证您的邮箱";
      url = `${url}/validate/change_email?uid=${userId}&authcode=${getAuthcode(
        userId
      )}`;
    } else if (type == "password") {
      title = "修改您的密码";
      url = `${url}/validate/change_password?uid=${userId}&authcode=${getAuthcode(
        userId
      )}`;
    }
  ...
  public static async validateChangeEmail(ctx: koa.Context) {
    ...
    if (user && authcode == getAuthcode(userId)) {
      let account: any = await Accounts.find({ where: { openid: user.email } });
      if (!account) {
        let accountData = {
          userId: userId,
          openid: user.email,
        ...
        account = await Accounts.create(accountData);
      }
      ctx.cookies.set(CookieKeys.USER_ID, user.id, cookieSetting);
      ctx.cookies.set(
        CookieKeys.REMEMBER_ME,
        getRememberMeToken(user.id),
        cookieSetting
      );
    }
    ctx.redirect("/");
